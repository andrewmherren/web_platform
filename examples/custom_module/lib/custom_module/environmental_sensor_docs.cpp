...
const std::vector<String> EnvironmentalSensorDocs::SENSOR_TAGS = {"Environmental Sensor"};

OpenAPIDocumentation EnvironmentalSensorDocs::createGetCurrentReadings() {
  OpenAPIDocumentation doc = OpenAPIFactory::create(
    "Get current sensor readings",
    "Returns current temperature and humidity readings from the environmental sensor",
...
OpenAPIDocumentation EnvironmentalSensorDocs::createUpdateSensorConfig() {
  OpenAPIDocumentation doc = OpenAPIFactory::create(
    ...
    "Updates sensor settings including location, thresholds, and enable/disable state",
  ...
  doc.requestExample = R"(location=Office&enabled=on&temp-threshold=30.0&humidity-threshold=60.0&alerts-enabled=on)";

  doc.responseExample = OpenAPIFactory::createSuccessResponse("Configuration updated successfully");
  doc.responseSchema = OpenAPIFactory::createSuccessResponse();
...
OpenAPIDocumentation EnvironmentalSensorDocs::createGetDetailedSensorData() {
  OpenAPIDocumentation doc = OpenAPIFactory::create(
    ...
    "Returns comprehensive sensor information including readings, configuration, and status alerts",
...
OpenAPIDocumentation EnvironmentalSensorDocs::createControlSensor() {
  OpenAPIDocumentation doc = OpenAPIFactory::create(
    ...
    "Send control commands to the sensor (refresh, enable, disable, reset-alerts)",
  ...
  doc.requestExample = R"(command=refresh)";

  doc.responseExample = OpenAPIFactory::createSuccessResponse("Sensor readings refreshed");
  doc.responseSchema = OpenAPIFactory::createSuccessResponse();
