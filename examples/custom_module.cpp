 ...
 *
 * This example shows how to create a custom web module that implements the
 * IWebModule interface. It demonstrates:
...
public:
  EnvironmentalSensorModule() { updateSensorReadings(); }
  ...
  String getModuleName() const override { return "Environmental Sensor"; }

  String getModuleVersion() const override { return "1.2.0"; }
...
private:
  ...
  void mainPageHandler(WebRequest &req, WebResponse &res) {
    String html =
        R"(
            ...
            String(temperature, 1) + R"(°C</p>
                            <small>Threshold: )" +
            String(tempThreshold, 1) + R"(°C</small>
                        </div>
            ...
            String(millis() / 1000) + R"( seconds ago</small>
                        </div>
                    </div>
                    
                    <div class="card">
                        <h2>Quick Actions</h2>
                        <button class="btn btn-primary" onclick="updateReadings()" >
        Refresh Data</ button><button class = "btn btn-secondary" onclick =
                                   "window.location='/sensors/config'">
                    Configuration</ button>
                <button class = "btn btn-info" onclick = "showAPIInfo()"> API
                    Information</ button></ div>

                <div id = "api-info" class = "card" style = "display:none;">
                <h2> API Endpoints</ h2><p>
                <strong> Public Access : </ strong></ p><ul><li><code> GET /
                                         sensors / api / current</ code> -
            Current sensor readings</ li></ ul><p><strong>
                    API Token Required : </ strong></ p><ul><li><code> GET /
                                         sensors / api / data</ code> -
            Detailed sensor data</ li><li>
                <code> POST / sensors / api / control</ code> -
            Control sensor operations</ li></ ul></ div></ div>

            <script> function showAPIInfo() {
      const apiInfo = document.getElementById('api-info');
      apiInfo.style.display =
          apiInfo.style.display == = 'none' ? 'block' : 'none';
    }
                </script>
            </body></html>
        )";
  ...
  void updateConfigHandler(WebRequest &req, WebResponse &res) {
    ...
    Serial.println("  Alerts: " +
                   String(alertsEnabled ? "Enabled" : "Disabled"));
