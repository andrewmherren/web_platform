...
<body>
  ...
  <script>
    ...
    // Get API URL base path relative to current location
    function getApiBasePath() {
      // Get current path and remove trailing slash if present
      let path = window.location.pathname;
      if (path.endsWith('/')) {
        path = path.slice(0, -1);
      }
      // If we're at the root of the module (like /wifi/ or /wifi/index.html)
      // then the API is at /wifi/api/*
      return path + '/api';
    }

    function loadWiFiStatus() {
      fetch(getApiBasePath() + '/status')
    ...
    function disconnectWiFi() {
      if (confirm('Are you sure you want to disconnect from WiFi?')) {
        fetch(getApiBasePath() + '/disconnect', {
          method: 'POST'
        })
        .then(response => response.json())
        .then(data => {
