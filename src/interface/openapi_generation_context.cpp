...
void OpenAPIGenerationContext::beginGeneration() {
    ...
    // CRITICAL FIX: Increase capacity to handle all API routes (23 found + growth)
    // The system has 23 API routes, so reserve 40 to be safe
    apiRouteDocs.reserve(40);
}

void OpenAPIGenerationContext::addRouteDocumentation(const String& path, WebModule::Method method, 
    ...
    if (isApiRoute || hasDocumentation) {
        ...
        if (apiRouteDocs.size() >= apiRouteDocs.capacity()) {
            ERROR_PRINTF("OpenAPIGenerationContext: Capacity exceeded (%d/%d), ignoring route %s\n",
