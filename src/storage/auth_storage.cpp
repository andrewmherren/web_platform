...
void AuthStorage::createDefaultAdminUser() {
    ...
    if (userKeys.empty()) {
        ...
        if (stored) {
            ...
            Serial.println("AuthStorage: ERROR - Failed to create default admin user");
        }
    }
}
...
void AuthStorage::initialize(const String& driver) {
    ...
    if (driverName.length() > 0) {
        ...
        if (!targetDriver) {
            Serial.printf("AuthStorage: Warning - driver '%s' not found, using default\n", 
                         driverName.c_str());
            driverName = "";
        }
    }
...
bool AuthStorage::updateUserPassword(const String& userId, const String& newPassword) {
    ensureInitialized();
...
bool AuthStorage::deleteUser(const String& userId) {
    ...
    if (success) {
        ...
        if (driverName.length() > 0) {
            sessionsQuery = QueryBuilder(StorageManager::driver(driverName), SESSIONS_COLLECTION);
        }
        sessionsQuery.where("userId", userId).remove();
        
        QueryBuilder tokensQuery = StorageManager::query(API_TOKENS_COLLECTION);
        if (driverName.length() > 0) {
            tokensQuery = QueryBuilder(StorageManager::driver(driverName), API_TOKENS_COLLECTION);
        }
...
String AuthStorage::validateCredentials(const String& username, const String& password) {
    ensureInitialized();
...
std::vector<AuthApiToken> AuthStorage::getUserApiTokens(const String& userId) {
    ...
    if (driverName.length() > 0) {
        query = QueryBuilder(StorageManager::driver(driverName), API_TOKENS_COLLECTION);
    }
...
bool AuthStorage::validatePageToken(const String& token, const String& clientIp) {
    ensureInitialized();
