...
struct AuthSession {
    ...
    AuthSession(const String& sessionId, const String& userId, const String& username);
...
struct AuthApiToken {
    String token;           // Token value (primary key) 
    String userId;          // Foreign key to AuthUser.id
    String username;        // Denormalized for quick access
    ...
    AuthApiToken(const String& token, const String& userId, const String& username, 
                 const String& name, unsigned long expireInDays = 0);
...
struct ConfigItem {
    String key;             // Setting name (primary key)
