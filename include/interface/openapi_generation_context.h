...
/**
 * OpenAPIGenerationContext - Temporary storage for OpenAPI documentation during generation
 *
 * This class provides temporary storage for route documentation that only exists
 * during the OpenAPI spec generation phase. This eliminates the permanent memory
 * overhead of storing documentation strings in RouteEntry structures.
 *
 * Memory Benefits:
...
public:
    ...
    struct RouteDocumentation {
        ...
        RouteDocumentation(const String& p, WebModule::Method m, 
                          const OpenAPIDocumentation& d, const AuthRequirements& auth)
